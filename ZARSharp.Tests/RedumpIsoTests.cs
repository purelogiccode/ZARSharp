...
/// <summary>
/// Black-box tests for Redump-aware <c>zar --iso</c> (MissingFeatures roll-up
/// #8): a Redump ISO must pack its game partition (wave-dependent offset),
...
public sealed class RedumpIsoTests
{
    ...
    internal static string? FindCli()
    {
        ...
        {
            if (File.Exists(Path.Combine(dir.FullName, "CSharp_ZARSharp.sln")))
            {
                ...
                foreach (var cfg in new[] { inferred, "Release", "Debug" })
                {
                    ...
                    {
                        var bin = Path.Combine(dir.FullName, "ZARSharp.Cli", "bin", cfg, tfm);
                        var apphost = Path.Combine(bin, OperatingSystem.IsWindows() ? "ZARSharp.Cli.exe" : "ZARSharp.Cli");
                        if (File.Exists(apphost))
                        {
                            return apphost;
                        }

                        var dll = Path.Combine(bin, "ZARSharp.Cli.dll");
                        if (File.Exists(dll))
                        {
                            return dll;
                        }
